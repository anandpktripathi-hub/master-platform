...
$pageRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/pages" -Method Post `
...
$templateRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/templates" -Method Post `
    -Headers @{"X-Tenant-ID"="demo-tenant"; "Content-Type"="application/json"} `
    -Body $templateBody
...
$useTemplateRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/templates/$templateId/use" -Method Post `
    -Headers @{"X-Tenant-ID"="demo-tenant"; "Content-Type"="application/json"} `
...
$auditRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/seo-audit/$pageId/run" -Method Post `
...
$trackRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/analytics/$pageId/track" -Method Post `
    -Headers @{"X-Tenant-ID"="demo-tenant"; "Content-Type"="application/json"} `
    -Body '{}'
...
$analyticsRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/analytics/page/$pageId" `
    -Headers @{"X-Tenant-ID"="demo-tenant"}

Write-Host "✅ Analytics retrieved: $($analyticsRes.Length) records`n"

Write-Host "=== TEST 7: Get Tenant Analytics ===" -ForegroundColor Green
$tenantAnalytics = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/analytics/tenant" `
    -Headers @{"X-Tenant-ID"="demo-tenant"}
...
$menuRes = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/menus/main/items" -Method Post `
    -Headers @{"X-Tenant-ID"="demo-tenant"; "Content-Type"="application/json"} `
    -Body $menuBody
...
$menuTree = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/menus/main/tree" `
    -Headers @{"X-Tenant-ID"="demo-tenant"}

Write-Host "✅ Menu tree retrieved with $($menuTree.Length) items`n"

Write-Host "=== TEST 10: Get Import History ===" -ForegroundColor Green
$importHistory = Invoke-RestMethod -Uri "http://localhost:4000/api/v1/cms/import/history" `
    -Headers @{"X-Tenant-ID"="demo-tenant"}
