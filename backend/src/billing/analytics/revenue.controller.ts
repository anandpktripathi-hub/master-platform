...
export interface BillingRevenueAnalyticsResponse {
  totalRevenueLast30: number;
  totalRevenueLast365: number;
  /**
   * Approximate Monthly Recurring Revenue based on the
   * most recent full month's paid invoice volume.
   */
  mrrApprox: number;
  arrApprox: number;
  ...
  async getRevenue(): Promise<BillingRevenueAnalyticsResponse> {
    ...
    const byMonth: BillingRevenueMonthBucket[] = monthlyAgg
      ...
      .map((row) => {
        ...
        } as BillingRevenueMonthBucket;
      })
      .sort((a, b) => (a.month < b.month ? -1 : a.month > b.month ? 1 : 0));

    const statusSummary: BillingRevenueStatusSummary = {
      paidLast30: 0,
      overdue: 0,
      cancelled: 0,
    };
