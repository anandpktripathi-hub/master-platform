...
import { Billing, BillingDocument } from '../../database/schemas/billing.schema';
...
export class BillingService {
  ...
  async create(createBillingDto: Billing, tenantId: string): Promise<Billing> {
    const createdBilling = new this.billingModel({ ...createBillingDto, tenantId });
    return createdBilling.save();
  }

  async update(id: string, updateBillingDto: Billing, tenantId: string): Promise<Billing> {
    return this.billingModel.findByIdAndUpdate(id, { ...updateBillingDto, tenantId }, { new: true }).exec();
  }
