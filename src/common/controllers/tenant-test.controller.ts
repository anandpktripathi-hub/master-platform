...
export class TenantTestController {
  ...
  @ApiOperation({ 
    ...
    description: 'Returns the current tenant context from the authenticated request. Use this to verify that tenantId is properly extracted from JWT and attached to the request.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  getTenantContext(@Request() req) {
    return {
      ...
      explanation: {
        middleware: 'TenantMiddleware extracts tenantId from JWT and attaches to req.tenantId',
        guard: 'TenantGuard validates tenantId exists and is valid',
        status: req.user?.tenantId === req.tenantId ? 'CONSISTENT ✅' : 'MISMATCH ❌',
      },
    };
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Returns success even without authentication. Tenant context should be undefined.'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  getPublic(@Request() req) {
    return {
      ...
      explanation: 'This endpoint does not require authentication, so tenantId should be undefined',
    };
  }
}
